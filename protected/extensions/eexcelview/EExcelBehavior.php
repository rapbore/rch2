...
class EExcelBehavior extends CBehavior
{
	 ...
	 */
	public function toExcel($model=null, $columns=array(), $title=null, $documentDetails=array(), $exportType='Excel2007', $services_name='reporte_egreso')
	{
		...
		elseif(is_array($model))
		{
			...
			if(empty($model)) {
				...
				Yii::log('Empty array passed as data, nothing to do *here*', 'info', 'system.base.CBehavior');
                                throw new CHttpException(400, Yii::t('app', 'No hay datos a exportar.'));
				return;
			} else {
		...
		$this->$services_name($arg);
		
	}
        
        public function reporte_egreso($argumentos=NULL){
            
            $this->owner->widget('ext.eexcelview.ReporteEgresoView', $argumentos);
            Yii::app()->end();
        }
}